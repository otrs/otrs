...
# --
# $origin: otrs - 488e602f7f7ee75e3d4fe855fe791aad3c2fc8a1 - Kernel/Modules/AgentDashboardCommon.pm
# --
...
# ---
            #--- LH-Mandatorywidgets start
# ---
            #--- The default widgets can not be removed .
            if ( $Config->{$Name}->{Default} && !$Config->{$Name}->{AllowDisableWidgets} ) {
                $Active = 1;
            }

# ---
            #--- LH-Mandatorywidgets end
# ---
            my $Key = $UserSettingsKey . $Name;
...
# ---
        #--- LH-Mandatorywidgets start
        # check  AllowDisableWidgets
# ---
        if ( !$Configs->{$Name}->{Default} || $Configs->{$Name}->{AllowDisableWidgets} ) {
            $LayoutObject->Block(
