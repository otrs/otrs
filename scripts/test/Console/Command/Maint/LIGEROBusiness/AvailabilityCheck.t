...
local *Kernel::System::LIGEROBusiness::LIGEROBusinessIsAvailable = sub {
    my ( $Self, %Param ) = @_;
...
# get needed objects
my $CommandObject    = $Kernel::OM->Get('Kernel::System::Console::Command::Maint::LIGEROBusiness::AvailabilityCheck');
...
my $NextUpdateTimeKey = 'LIGEROBusiness::AvailabilityCheck::NextUpdateTime';

# delete the 'LIGEROBusiness::AvailabilityCheck::NextUpdateTime' from the system data, if it already exists in the system
...
$Self->Is(
    $ExitCode,
    0,
    "Maint::LIGEROBusiness::AvailabilityCheck exit code",
);

$Self->True(
    $TestCloudServiceCall,
    "The function 'LIGEROBusinessIsAvailable' was called from the console command.",
);
...
$Self->False(
    $TestCloudServiceCall,
    "The function 'LIGEROBusinessIsAvailable' was not called from the console command.",
);
...
$Self->True(
    $TestCloudServiceCall,
    "The function 'LIGEROBusinessIsAvailable' was called from the console command (with --force).",
);
...
$Self->True(
    $TestCloudServiceCall,
    "The function 'LIGEROBusinessIsAvailable' was called from the console command (because next update time reached).",
);
