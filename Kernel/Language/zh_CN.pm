...
sub Data {
    ...
    $Self->{Translation} = {
        ...
        'Tickets in My Services' => '工单在我的服务',
        'Service Time' => '服务时间',
        ...
        'Scheduler is an OTRS separated process that perform asynchronous tasks' =>
            'OTRS 很多同步执行的任务都依靠调度程序处理',
        '(e.g. Generic Interface asynchronous invoker tasks)' => '例如 Generic Interface 等需要同步执行的任务',
        'It is necessary to have the Scheduler running to make the system work correctly!' =>
            '只有调度程序正常运行的情况下才可以确保系统正常运作！',
        'Starting Scheduler' => '如何启动调度程序',
        'Make sure that %s exists (without .dist extension)' => '确认 %s 文件已经在存（.dist 除外）',
        'Check that cron deamon is running in the system' => '检查系统 cron 进程是否已经在运行',
        'Confirm that OTRS cron jobs are running, execute %s start' => '执行 %s，确保 OTRS 的 cron 在运行',
        ...
        'Send me a notification if a customer sends a follow up and I\'m the owner of the ticket or the ticket is unlocked and is in one of my queues/services.' =>
            '当新客户回信触发跟进时，并且我是工单的所有者或未锁定的工单在我的队列或服务时请通知我。',
        'Send me a notification if the service of a ticket is changed to a service in "My Services" and the ticket is in a queue where I have read permissions.' =>
            '当工单进入“我的服务”，并且工单所在的队列我有读的权限时，请通知我。',
        'Send me a notification if there is a new ticket in my queues/services.' =>
            '当新的工单进入属于我的队列或服务时请通知给我。',
        'Send new ticket notifications if subscribed to' => '发送新的工单通知',
        'Send notifications to users.' => '给用户和服务人员发送通知',
        'Send service update notifications' => '发送服务更新给我',
        'Send ticket follow up notifications if subscribed to' => '发送工单跟进通知',
        ...
        'Your service selection of your favorite services. You also get notified about those services via email if enabled.' =>
            '选中你所善长的服务，如果你打开邮件通知功能时，选中的服务里的工单有变化时将通知你。',
