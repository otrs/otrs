        ...
        if ( !IsHashRefWithData($ProcessData) ) {
            return $LayoutObject->ErrorScreen(
                Message => $LayoutObject->{LanguageObject}
                    ->Translate('Could not get data for ProcessID %s', $ProcessID),
            );
        }
        ...
        if ( $CheckResult->{Success} ) {
            ...
            if ( !$Success ) {
                $DeleteResult{Message} = $LayoutObject->{LanguageObject}
                    ->Translate('Process: %s could not be deleted', $ProcessID);
            }
            else {
            ...
            if ( $Entity->{EntityID} ne $GetParam{EntityID} ) {
                $JSON = $LayoutObject->JSONEncode(
                    Data => {
                        Success => 0,
                        Message => $LayoutObject->{LanguageObject}
                            ->Translate('The %s:%s has a different EntityID', $GetParam{EntityType}, $GetParam{ItemID}),
                    },
                );
            }
            else {
        ...
        if ( !IsHashRefWithData($EntityData) ) {
            $JSON = $LayoutObject->JSONEncode(
                Data => {
                    Success => 0,
                    Message => $LayoutObject->{LanguageObject}
                        ->Translate('Could not get %s', $GetParam{EntityType}),
                },
            );
        }
        else {
...
sub _CheckProcessDelete {
    ...
    if ( !IsHashRefWithData($ProcessData) ) {
        return {
            Success => 0,
            Message => $LayoutObject->{LanguageObject}
                ->Translate('Could not get data for ProcessID %s', $Param{ID}),
        };
    }
    ...
    if ( $State ne 'Inactive' ) {
        return {
            Success => 0,
            Message => $LayoutObject->{LanguageObject}
                ->Translate('Process: %s is not Inactive', $Param{ID}),
        };
    }
