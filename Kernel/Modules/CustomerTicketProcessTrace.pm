...
package Kernel::Modules::CustomerTicketProcessTrace;
...
sub Run {
    ...
    if ( !$Access ) {
        return $LayoutObject->CustomerNoPermission(
            Message    => "You need rw permissions!",
            WithHeader => 'yes',
        );
    }
    ...
    # get customer of ticket
    my $ProcessOperator = $Ticket{CustomerUserID};

    if ($ProcessOperator) {
        my %CustomerUserData = $Kernel::OM->Get('Kernel::System::CustomerUser')->CustomerUserDataGet(
            User  => $ProcessOperator,
            Valid => 1
        );

        $ProcessOperator = $CustomerUserData{UserFullname};
    }
    ...
    for my $Data (@Lines) {
        ...
        if ( $Data->{Name} && $Data->{Name} =~ m/^%%/x ) {
                ...
                )
            {
                ...
                # check history records , choose the record which oldValue equals null,that means the first record.
                if ( !( $Values[5] ) ) {

                    my $OperatorName = $Data->{UserFullname};
                    ...
                    if ( !$ProcessOperator ) {
                        $ProcessOperator = $OperatorName;
                    }

                    $ProcessInformation{Operator}     = $ProcessOperator;
                    ...
                    if ( $Variable == 0 ) {
                        ...
                        # the other records
                        $ProcessInformation{Operator}     = $OperatorName;
                    ...
                    # consume time
                    # get datetime object
                    my $ProcessUpdateStartTime = $Kernel::OM->Create(
                        'Kernel::System::DateTime',
                        ObjectParams => {
                            String => $ProcessHistory[$Variable]{StartTime},
                        },
                    )->ToEpoch();
                    my $ProcessUpdateEndTime = $Kernel::OM->Create(
                        'Kernel::System::DateTime',
                        ObjectParams => {
    ...
    $Output .= $LayoutObject->Output(
        TemplateFile => 'CustomerTicketProcessTrace',
        Data         => {
