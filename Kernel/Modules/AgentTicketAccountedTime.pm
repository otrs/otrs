...
sub Run {
    ...
    unless ( $Self->{ConfigObject}->Get('Ticket::Frontend::AccountTime') &&
         $Self->{ConfigObject}->Get('MuehlbauerTickets')->{'AgentAccountedTime'} ) {
         return $Self->{LayoutObject}->ErrorScreen(
    ...
    if ( $Ticket{TotalAccountedTime} ) {
        ...
        # convert time to hour notation
        $Ticket{TotalAccountedTimeHours} = int ( $Ticket{TotalAccountedTime} / 60 );
        $Ticket{TotalAccountedTimeMinutes} = $Ticket{TotalAccountedTime} % 60;
