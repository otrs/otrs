...
sub QueryCondition {
    ...
    # escape "###NNN###" with pre-backslash as use it for processing quote
    # double escape and ".+?" expressions in following step
    $Param{Value} =~ s/(?<!\\)((?:\\\\)*)(###\d+###)/$1\\$2/g;

    # quote ".+?" expressions
    # for example ("some and me" AND !some), so "some and me" is used for search 1:1,
    # but \"some and me", "some and me\" should not be escaped
    $Param{Value} =~ s{
        (?<!\\)((?:\\\\)*)"((?:.*?[^\\])(?:\\\\)*)(?:"|$)
    }
    {
    ...
    # quote double escape expressions
    $Param{Value} =~ s{
        (?<!\\)((?:\\\\)+)
    }
    {
    ...
    # fix usage both AND and OR conditions, double conditions and
    # leading/trailing conditions until they disappear
    do {
        $Prev = $Param{Value};

        # replace usage both AND and OR conditions with OR conditions
        # for example a && || b is replaced with a || b, as it should be
        # interpreted as ( a && ) || b
        $Param{Value} =~ s/(?<!\\)\|\|\s*&&|(?<!\\)&&\s*\|\|/||/g;
    ...
    # pick words in order, and take them into SQL
    # for example (aa || bb) && cc is picked as (, aa, ||, bb, ), &&, cc
    WORD:
    for my $Word (
        $Param{Value} =~ m{
            \s*
